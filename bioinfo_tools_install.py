...
def installSamtools():
  ...
  proc = subprocess.run('cp '+TEMPORAL+'/samtools/develop/examples/ex1.fa '+REFERENCE_DIR, shell=True)
  proc = subprocess.run('gunzip '+TEMPORAL+'/samtools/develop/examples/ex1.sam.gz', stdout=PIPE, stderr=PIPE, shell=True)
  proc = subprocess.run('cp '+TEMPORAL+'/samtools/develop/examples/ex1.sam ' +TEST_DIR, stdout=PIPE, stderr=PIPE, shell=True)
  proc = subprocess.run('rm -r '+TEMPORAL+'/samtools*', shell=True)
  os.chdir(WORK_SPACE)
  proc = subprocess.run('samtools fastq '+TEST_DIR+'/ex1.sam > '+TEST_DIR+'/ex1.fq', shell=True, stdout=PIPE, stderr=PIPE, text=True)
